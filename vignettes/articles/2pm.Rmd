...
The input arguments for the `dpit_2pm()` function differ from those of other functions in `assessor` package. Specifically, users can utilize this function with either models or Probability Integral Transform (PIT) as input. The function returns a `dpit` object; `residuals()`, `summary()`, and `plot()` extract, summarize, and plot its residuals on the selected scale.
