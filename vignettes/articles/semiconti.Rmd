...
`dpit()` is used for calculating the DPIT residuals for regression models with semicontinuous outcomes, and `plot()` constructs corresponding QQ-plots from the returned objects. Specifically, a Tobit regression and a Tweedie regression model are suitable models for `dpit()`. The suitable model objects are as follows:
...
#### Tobit
`dpit()` function supports calculating DPIT residuals for a Tobit regression from both `VGAM::vglm` and `AER::tobit` packages.
