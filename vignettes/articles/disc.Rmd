...
`dpit()` is used for calculating the DPIT residuals for regression models with discrete outcomes and constructing their QQ-plots. The suitable model objects are as follows: 
...
#### Negative binomial
An example of the usage of the `dpit()` function for a negative binomial regression is included below. 
...
#### Ordinal
Our `dpit()` function is also applicable to ordinal regression fitted by `MASS::polr()`. 
