...
Calculates the DPIT residuals for a regression model with zero-inflated
discrete outcome. A zero-inflated model from `pscl` is used in this
function.

## Usage

``` r
resid_zeroinfl(model, plot=TRUE, scale='normal', line_args=list(), ...)
```
